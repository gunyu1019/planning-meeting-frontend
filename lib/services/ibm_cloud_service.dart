...
class IBMCloud {
  ...
  Future<IBMAuthentic> identityToken() async {
    final body =
        'grant_type=urn:ibm:params:oauth:grant-type:apikey&apikey=$__apikey';
  ...
  Future<WatsonX> getWatsonX(String instanceUrl) async {
    if (__cached_iam_token == null ||
        DateTime.now().compareTo(__cached_iam_token!.expiration) > 0) {
      await identityToken();
    }
